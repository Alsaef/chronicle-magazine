...
// -------------------------------------------------------------
// In-Memory Cache & Inflight Promise Deduplication
// -------------------------------------------------------------
...
async function dedupeFetch(cacheKey, ttlMs, fetchFn) {
  const now = Date.now();
  if (ttlMs > 0 && memoryCache.has(cacheKey)) {
...
export async function getStories({ category, featured, sort, search, limit } = {}) {
  ...
  // Cache non-search queries for 20s; bypass memory cache on active text searches
  const ttl = search ? 0 : 20000;

  return dedupeFetch(cacheKey, ttl, async () => {
    const res = await fetch(`${API_BASE}/stories?${queryString}`);
    return handleResponse(res);
  });
}

export async function getCategories() {
  return dedupeFetch('categories', 60000, async () => {
...
export async function getStory(idOrSlug) {
  const cacheKey = `story:${idOrSlug}`;
  return dedupeFetch(cacheKey, 15000, async () => {
...
export async function likeStory(idOrSlug, token) {
  // Invalidate story cache so new like totals are visible immediately
  clearApiCache('stories');
  clearApiCache(`story:${idOrSlug}`);
